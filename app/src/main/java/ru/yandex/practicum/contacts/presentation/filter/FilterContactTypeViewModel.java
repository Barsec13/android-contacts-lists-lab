...
public class FilterContactTypeViewModel extends BaseBottomSheetViewModel {
    ...
    private void updateFilterContactTypes() {
        ...
        // создайте массив строк. Для инициализации используйте метод
        // ContactType.getContactTypes(), который должен возвращать
        // список всех доступных источников контактов
        String[] contactTypes = ContactType.getContactTypes();
        FilterContactTypeUi all = createAllSelectedItem(contactTypes);
        ...
        filterContactTypeUis.add(all);
        for (String contact:contactTypes){
            filterContactTypeUis.add(createFilterContactType(contact));
        }
        // вызовите меотод setValue() у переменной filterContactTypesLiveDate и передайте
        // в качестве аргументы ваш список типа FilterContactTypeUi
